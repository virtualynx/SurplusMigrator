            ...
            foreach(RowData<ColumnName, object> data in inputs) {
                ...
                if(advertiserid != 0) {
                    try {
                        ...
                        nullAdvertiserOrBrandErrors.Add(new DbInsertFail() {
                            info = "Missing reference in table (" + missingReferenceAdvertiser.referencedTableName + "), value (" + missingReferenceAdvertiser.referencedColumnName + ")%(" + data + ") is not exist",
                ...
                if(advertiserbrandid != 0) {
                    try {
                        ...
                        nullAdvertiserOrBrandErrors.Add(new DbInsertFail() {
                            info = "Missing reference in table (" + missingReferenceBrand.referencedTableName + "), value (" + missingReferenceBrand.referencedColumnName + ")%(" + data + ") is not exist",
                            severity = DbInsertFail.DB_FAIL_SEVERITY_WARNING,
                            type = DbInsertFail.DB_FAIL_TYPE_FOREIGNKEY_VIOLATION,
